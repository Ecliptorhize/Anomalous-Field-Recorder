...
def cmd_profile(args: argparse.Namespace) -> None:
    ...
    if args.output:
        ...
        print(f"[profile] saved profile to {output_path}")
    else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    profile = sub.add_parser("profile", help="Profile a dataset for stats and sampling metadata")
    profile.add_argument("dataset", help="Path to dataset (CSV/JSON/Parquet)")
    profile.add_argument("--timestamp-column", default="timestamp", help="Timestamp column for sample-rate inference")
    profile.add_argument("--value-column", action="append", help="Numeric value column(s) to prioritize")
    profile.add_argument("--sample-rate", type=float, help="Override sample rate when timestamps missing")
    profile.add_argument("--output", help="Optional path to write profile JSON")
